...
class FineDiffTest(unittest.TestCase):
    ...
    def test_substantive_change_is_reported(self):
        ...
        self.assertTrue(any(op["op"] == "replace" for op in got["ops"]))

    def test_reports_added_and_removed_words(self):
        self.assertTrue(any(o["op"] == "insert" for o in fine_diff("a b", "a x b")["ops"]))
        self.assertTrue(any(o["op"] == "delete" for o in fine_diff("a x b", "a b")["ops"]))
    ...
    def test_wholly_different_text_is_a_single_replace(self):
        ...
        got = fine_diff(a, b)
        self.assertEqual(len(got["ops"]), 1)
        self.assertEqual(got["ops"][0]["op"], "replace")
    ...
    def test_op_list_is_capped_when_differences_interleave(self):
        ...
        got = fine_diff(a, b)
        self.assertEqual(len(got["ops"]), 200)
